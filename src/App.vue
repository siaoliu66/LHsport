...
const day=ref(['Push','Pull','Legs'].includes(stored.day)?stored.day:'Push')
...
function applyRemoteCatalog(exerciseRows,programRows){
 ...
 const exMap=new Map(exRows.map(r=>[String(r.exerciseId),r])),next={beginner:{Push:[],Pull:[],Legs:[]},advanced:{Push:[],Pull:[],Legs:[]}},nextGuides={}
...
<template v-if="view==='train'"><div class="heading"><div><small class="overline">TODAY'S SESSION</small><h1>{{day}} <span>訓練</span></h1></div><div class="progress"><strong>{{done}}<span>/{{total}}</span></strong><small>已完成組數</small></div></div>
<div class="choices"><div class="switch"><button :class="{selected:level==='beginner'}" @click="choose('beginner',day)">新手 · 約 40 分鐘</button><button :class="{selected:level==='advanced'}" @click="choose('advanced',day)">非新手</button></div><div class="switch"><button v-for="d in ['Push','Pull','Legs']" :key="d" :class="{selected:day===d}" @click="choose(level,d)">{{d}}</button></div></div>
